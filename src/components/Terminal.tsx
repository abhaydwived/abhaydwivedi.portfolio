...
// ---------------------------------------------------------------------------
// Internal class — same logic as terminal.ts TerminalResume, fully typed
// ---------------------------------------------------------------------------
class TerminalResume {
    ...
    private p5Instance: unknown;
    private matrixInterval: ReturnType<typeof setInterval> | null;
    ...
    constructor() {
        ...
        this.terminals = this.input
            ? [{ input: this.input, history: [], historyIndex: -1 }]
            : [];
    ...
    private setupContextMenu() {
        this.terminalContainer?.addEventListener('contextmenu', (e) => {
            e.preventDefault();
            const terminalContent = (e.target as HTMLElement).closest('.terminal-content') as HTMLElement | null;
            if (terminalContent) {
    ...
    private showContextMenu(x: number, y: number) {
        ...
        const closeOption = this.contextMenu.querySelector('[data-action="close-split"]') as HTMLElement | null;
    ...
    private handleContextMenuAction(action: string) {
        ...
        switch (action) {
            case 'split-h': this.splitTerminal('horizontal', this.activeTerminalContent); break;
            case 'split-v': this.splitTerminal('vertical', this.activeTerminalContent); break;
            case 'close-split': this.closeSplit(this.activeTerminalContent); break;
        }
        this.contextMenu?.classList.remove('active');
    }
    ...
    private setupEventListeners() {
        this.terminalContainer?.addEventListener('click', (e) => {
            ...
            if (terminalContent) {
                const input = terminalContent.querySelector('input') as HTMLInputElement | null;
                if (input) {
                    input.focus();
        ...
        document.addEventListener('keydown', (e) => {
            if (e.ctrlKey && e.shiftKey && e.key.toLowerCase() === 'h') {
                e.preventDefault();
                const activeContent = this.terminals[this.activeTerminal]?.input.closest('.terminal-content') as HTMLElement | null;
    ...
    private setupInputHandlers(inputElement: HTMLInputElement) {
        inputElement.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'Enter') {
                ...
                e.preventDefault();
                const outputElement = inputElement.closest('.terminal-content')?.querySelector("[id^='output']") as HTMLElement | null;
                if (outputElement) {
    ...
    private handleTabCompletion(inputElement: HTMLInputElement) {
        ...
        if (matches.length === 1) {
            ...
            const outputElement = inputElement.closest('.terminal-content')?.querySelector("[id^='output']") as HTMLElement | null;
            if (outputElement) this.printToOutput(outputElement, `\nPossible commands:\n${matches.join('  ')}`, 'info');
        }
    }

    private navigateHistory(direction: 'up' | 'down', terminal: TerminalState) {
    ...
    private splitTerminal(direction: 'horizontal' | 'vertical', sourceTerminal: HTMLElement) {
    ...
    private createNewTerminalContent(container: HTMLElement) {
    ...
    private setupResizeHandle(handle: HTMLElement) {
        ...
        const resize = (e: MouseEvent) => {
            ...
            const r = this.resizing as { parentContainer: Element; element: HTMLElement; startX: number; startY: number; initialSize: number };
            const containerRect = r.parentContainer.getBoundingClientRect();
            if (isHorizontal) {
                const newWidth = r.initialSize + (e.clientX - r.startX);
                const maxWidth = containerRect.width - 150;
                if (newWidth >= 150 && newWidth <= maxWidth) {
                    r.element.style.flex = 'none';
                    r.element.style.width = `${(newWidth / containerRect.width) * 100}%`;
                }
            } else {
                const newHeight = r.initialSize + (e.clientY - r.startY);
                const maxHeight = containerRect.height - 100;
                if (newHeight >= 100 && newHeight <= maxHeight) {
                    r.element.style.flex = 'none';
                    r.element.style.height = `${(newHeight / containerRect.height) * 100}%`;
                }
            }
        };
    ...
    private printToOutput(outputElement: HTMLElement, text: string, className = '', useTypewriter = false): Promise<void> {
        ...
        if (useTypewriter && !text.includes('<')) return this.typeText(line, text, 20);
        if (useTypewriter && text.includes('<')) return this.typeHTML(line, text, 20);
        line.innerHTML = text;
        return Promise.resolve();
    }

    private scrollToBottom(terminalContent: HTMLElement) {
    ...
    private handleCommand(inputElement: HTMLInputElement) {
        const terminal = this.terminals.find(t => t.input === inputElement);
        if (!terminal) return;
    ...
    private printWelcomeMessage(outputElement: HTMLElement = this.output!) {
...
╚═╝     ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝╚═╝ ╚═════╝ `;
        ...
        const welcome =
            ...
            this.wrapWithColor(divider + '\n\n', '#555555') +
            this.wrapWithColor("Type ", '#666666') + this.wrapWithColor("'help'", '#87af87') +
            this.wrapWithColor(' to see available commands\n', '#666666') +
            this.wrapWithColor('Press ', '#666666') + this.wrapWithColor("'tab'", '#87af87') +
    ...
    private showHelp(outputElement: HTMLElement = this.output!) {
        if (!outputElement) return;
    ...
    private showAbout(outputElement: HTMLElement = this.output!) { if (!outputElement) return; }
    private showExperience(outputElement: HTMLElement = this.output!) { if (!outputElement) return; }
    private showEducation(outputElement: HTMLElement = this.output!) { if (!outputElement) return; }
    private showSkills(outputElement: HTMLElement = this.output!) { if (!outputElement) return; }
    private showContact(outputElement: HTMLElement = this.output!) { if (!outputElement) return; }

    private wrapWithColor(text: string, color: string): string {
        return `<span style="color: ${color}">${text}</span>`;
    }

    private typeText(element: HTMLElement, text: string, speed = 30): Promise<void> {
    ...
    private async typeHTML(element: HTMLElement, html: string, speed = 30): Promise<void> {
    ...
    private closeSplit(terminalContent: HTMLElement) {
    ...
    private showModal(modal: HTMLElement) { modal.classList.add('active'); }
    private closeModal(modal: HTMLElement) { modal.classList.remove('active'); }
    ...
    private showProjects() {
        ...
        container.innerHTML = this.projects.map(() => `...`).join('');
        this.showModal(this.projectsModal);
    }

    private showSkillsVisualization() {
        if (!this.skillsModal) return;
        const container = this.skillsModal.querySelector('.skills-container') as HTMLElement;
        container.innerHTML = Object.entries(this.skills).map(() => `...`).join('');
        this.showModal(this.skillsModal);
    }

    private generatePDF() { /* PDF generation logic */ }

    private initGame() {
        this.endGame();
        const outputElement = this.terminals[this.activeTerminal]?.input.closest('.terminal-content')?.querySelector("[id^='output']") as HTMLElement | null;
        if (outputElement) {
    ...
    private endGame() {
        if (this.p5Instance && typeof (this.p5Instance as { remove: () => void }).remove === 'function') {
            (this.p5Instance as { remove: () => void }).remove();
        }
        document.getElementById('snake-game-container')?.remove();
    }

    private initSnakeGame() { /* p5.js snake game */ }

    private startMatrixEffect(outputElement: HTMLElement) {
    ...
    private stopMatrixEffect() {
        if (this.matrixInterval) clearInterval(this.matrixInterval);
        document.getElementById('matrix-container')?.remove();
    }

    private async showWeather(_location: string, _outputElement: HTMLElement) { /* fetch logic */ }
    private calculate(_expression: string, _outputElement: HTMLElement) { /* eval logic */ }
    private generateLinkedInCover(_outputElement: HTMLElement) { /* DOM construction */ }
 ...
 * Converted from terminal.ts `TerminalResume` class.
 * No JSX output — operates on existing `terminal.html` DOM elements.
 */
export function Terminal(): null {
    useEffect(() => {
        const terminal = new TerminalResume();
